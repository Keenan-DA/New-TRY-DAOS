-- ============================================
-- FIX v_pipeline_funnel COLUMN NAMES
-- Adds aliases to match what HTML expects
-- ============================================
...
ai_opt_outs AS (
    SELECT ad.location_id, COUNT(DISTINCT ad.contact_id) AS opt_outs
    FROM ai_decisions ad WHERE ad.action = 'remove' GROUP BY ad.location_id
),
rep_removes AS (
    SELECT r.location_id, COUNT(DISTINCT r.contact_id) AS rep_removed
    FROM reactivations r WHERE r.action = 'remove' GROUP BY r.location_id
)
SELECT
    ls.location_id,
    d.dealership_name,
    ...
    -- ALIAS for HTML: reply_rate = response_rate
    ROUND(100.0 * COALESCE(ls.total_responses, 0) / NULLIF(COALESCE(ls.outbound_sent, 0), 0), 1) AS reply_rate,
    -- Opt-outs
    ...
    -- ALIAS for HTML: appointments_booked = leads_with_appointments
    COALESCE(la.leads_with_appointments, 0) AS appointments_booked,
