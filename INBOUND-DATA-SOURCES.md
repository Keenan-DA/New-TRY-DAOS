...
1. [Quick Reference: Data Source â†’ Table Map](#1-quick-reference)
...
| Column | Expected Value |
|--------|----------------|
| `created_source` | `'ai_automated'` â† **KEY IDENTIFIER** |
...
| Column | Expected Value |
|--------|----------------|
| `created_source` | `'rep_instructed'` â† **KEY IDENTIFIER** |
...
### âš ï¸ IMPORTANT: AI Remove vs Rep Remove
...
| Check | Expected |
|-------|----------|
| `ai_decisions` WHERE action='task' | â‰ˆ `tasks` count |
| `ai_decisions` WHERE action='appointment' | â‰ˆ `appointments` WHERE created_source='ai_automated' |
| `reactivations` WHERE action='appointment' | â‰ˆ `appointments` WHERE created_source='rep_instructed' |
