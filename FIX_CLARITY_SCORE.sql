-- ============================================================================
-- FIX: Clarity Score Calculation in v_health_score
-- ============================================================================
-- PROBLEM: clarity_score is showing ~6.8% average when it should be ~83.5%
-- ROOT CAUSE: The view is likely not properly aggregating instruction clarity
--
-- Run this SQL in Supabase SQL Editor to fix the views
-- ============================================================================

-- Step 1: First, let's verify the issue by checking current clarity data
-- Run this SELECT first to confirm the problem:

SELECT
    'v_instruction_clarity aggregation' as source,
    COUNT(*) as total_instructions,
    COUNT(*) FILTER (WHERE clarity_level = 'complete') as complete_count,
    COUNT(*) FILTER (WHERE clarity_level = 'empty') as empty_count,
    ROUND(
        100.0 * COUNT(*) FILTER (WHERE clarity_level = 'complete') /
        NULLIF(COUNT(*) FILTER (WHERE clarity_level != 'empty'), 0)
    , 1) as expected_clarity_pct
FROM v_instruction_clarity;

-- ============================================================================
-- Step 2: DROP and recreate v_health_score with CORRECT clarity calculation
-- ============================================================================
...
-- ============================================================================
-- Step 3: Recreate v_cs_account_health with all the additional columns
-- ============================================================================
...
SELECT
    ...
    -- Days since activity
    GREATEST(
        EXTRACT(DAY FROM NOW() - COALESCE(a.last_task_created, NOW() - INTERVAL '999 days')),
        EXTRACT(DAY FROM NOW() - COALESCE(ra.last_reactivation, NOW() - INTERVAL '999 days')),
        EXTRACT(DAY FROM NOW() - COALESCE(aa.last_appointment_booked, NOW() - INTERVAL '999 days'))
    ...
    -- Health trend placeholder (would need historical data)
    'STABLE' as health_trend,
    0 as loop_closure_change,
...
-- Run this to check the new scores:
SELECT
    'AFTER FIX' as status,
...
-- avg_clarity_score should jump from ~6.8% to ~75-85%
-- avg_adoption_score should jump from ~26 to ~55-65
-- CRITICAL accounts should decrease significantly
-- ============================================================================
